...
class IsolationApi {
  ...
  async create(saveData) {
    try {
      const response = await AuthorizationAxios.put(
        process.env.REACT_APP_BASE_URL + "/api/admission/quarantine/save",
        JSON.stringify({
  ...
  async charge(admissionId) {
    try {
      const response = await AuthorizationAxios.patch(
        process.env.REACT_APP_BASE_URL + "/api/admission/quarantine/charge",
        JSON.stringify({
          admissionListSearchByQuarantineVO: {
            patientId: this.searchPatientId.current.value,
            patientNm: this.searchPatientNm.current.value,
  ...
  async discharge(admissionId, dischargeDate, quantLocation, patientState) {
    try {
      const response = await AuthorizationAxios.patch(
        process.env.REACT_APP_BASE_URL + "/api/admission/quarantine/discharge",
        JSON.stringify({
          admissionListSearchByQuarantineVO: {
            patientId: this.searchPatientId.current.value,
            patientNm: this.searchPatientNm.current.value,
